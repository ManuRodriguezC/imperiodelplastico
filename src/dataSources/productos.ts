export const products = [
  {
    nombre: "Bolsa de Basura Negra",
    descripcion: "Bolsa resistente para desechos, ideal para uso doméstico e industrial.",
    imagen: "/images/products/bolsa_basura.webp",
    slug: "bolsa-basura_negra",
    medidas: ["16x24 cm", "20x28 cm", "22x32 cm", "24x36 cm", "28x40 cm", "30x40 cm"],
    favorite: true,
    linea: ["bolsas"]
  },
  {
    nombre: "Bolsas Ziploc",
    descripcion: "Bolsas resellables ideales para conservar alimentos y otros productos.",
    imagen: "/images/products/ziploc.webp",
    slug: "ziploc",
    medidas: ["3x4 cm", "x5 cm", "5x6 cm", "6x9 cm", "8x10 cm", "10x14 cm", "14x14 cm", "22x18 cm", "25x36 cm"],
    price: "$ 25.000",
    favorite: false,
    linea: ["bolsas"]
  },
  {
    nombre: "Bolsas Polipropileno",
    descripcion: "Bolsa de polipropileno resistente, reutilizable y versátil para embalaje y almacenamiento seguro.",
    imagen: "/images/products/bolsa_poliprofileno.webp",
    slug: "bolsas-poliprofileno",
    medidas: ["4x8 cm", "5x9 cm", "7x10 cm", "8x12 cm", "9x14 cm", "10x16 cm", "12x14 cm", "14x20 cm", "16x24 cm"],
    favorite: false,
    linea: ["bolsas"]
  },
  {
    nombre: "Bolsas de Seguridad",
    descripcion: "Bolsa de seguridad con cierre inviolable, ideal para transporte y resguardo de documentos o valores.",
    imagen: "/images/products/bolsa_seguridad.webp",
    slug: "bolsas-seguridad",
    medidas: ["16x24 cm", "23x34 cm", "30x40 cm", "30x50 cm"],
    favorite: true,
    linea: ["bolsas"]
  },
  {
    nombre: "Bolsas de Papel",
    descripcion: "Bolsas ecológicas de papel, ideales para tiendas y supermercados.",
    imagen: "/images/products/bolsas_papel.webp",
    slug: "bolsas-papel",
    medidas: ["8x16 cm", "10x20 cm", "12x22 cm", "14x28 cm", "16x32 cm", "18x35 cm", "20x40 cm", "24x45 cm", "28x50 cm", "32x55 cm"],
    favorite: true,
    linea: ["bolsas"]
  },
  
  {
    nombre: "Bolsa Celofán",
    descripcion: "Bolsas de celofán transparentes, ideales para empaquetar prendas de vestir, regalos, documentos y más.",
    imagen: "/images/products/bolsa_celofan.webp",
    slug: "bolsa-celofan",
    medidas: ["20.32x27.94 cm"],
    favorite: true,
    linea: ["bolsas"]
  },
  {
    nombre: "Cinta adhesiva",
    descripcion: "Cinta adhesiva multiusos para embalaje y sellado.",
    imagen: "/images/products/cinta_adhesiva.webp",
    slug: "cinta-adhesiva",
    medidas: ["50 mts", "100 mts", "200mts", "300mts", "500mts", "700mts"],
    favorite: false,
    linea: ["cintas"]
  },
  {
    nombre: "Cinta de peligro",
    descripcion: "Cinta de advertencia para señalización en zonas de riesgo.",
    imagen: "/images/products/cintapeligro.webp",
    slug: "cinta-peligro",
    medidas: ["7 cmx 100 m"],
    price: "$ 25.000",
    favorite: true,
    linea: ["seguridad"]
  },
  {
    nombre: "Cinta termoaislante",
    descripcion: "Cinta térmica para control de temperatura en almacenes y oficinas.",
    imagen: "/images/products/cinta_termo.webp",
    slug: "cinta-termoaislante",
    medidas: ["2x2 m"],
    price: "$ 25.000",
    favorite: false,
    linea: ["cintas"]
  },
  {
    nombre: "Gorro desechable",
    descripcion: "Gorro protector desechable para uso en industrias y hospitales.",
    imagen: "/images/products/gorro.webp",
    slug: "gorro-desechable",
    medidas: ["Talla única"],
    price: "$ 25.000",
    favorite: true,
    linea: ["seguridad"]
  },
  {
    nombre: "Guantes de nitrilo",
    descripcion: "Guantes resistentes y libres de látex para uso médico e industrial.",
    imagen: "/images/products/guantes_de_nitrilo.webp",
    slug: "guantes-nitrilo",
    medidas: ["S, M, L, XL"],
    price: "$ 25.000",
    favorite: true,
    linea: ["seguridad"]
  },
  {
    nombre: "Tapabocas",
    descripcion: "Mascarilla desechable de protección personal.",
    imagen: "/images/products/tapabocas.webp",
    slug: "tapabocas",
    medidas: ["Talla única"],
    price: "$ 25.000",
    favorite: false,
    linea: ["seguridad"]
  },
  {
    nombre: "Lona payaso",
    descripcion: "Lona colorida y resistente para múltiples usos.",
    imagen: "/images/products/lona_payaso.webp",
    slug: "lona-payaso",
    medidas: ["50x80 cm", "60x100 cm", "70x110 cm", "80x120 cm", "100x150 cm"],
    price: "$ 25.000",
    favorite: false,
    linea: ["agro"]
  },
  {
    nombre: "Mantel decorativo",
    descripcion: "Mantel elegante y duradero para eventos y restaurantes.",
    imagen: "/images/products/mantel_decorativo.webp",
    slug: "mantel-decorativo",
    medidas: ["1.5x2 m"],
    price: "$ 25.000",
    favorite: true,
    linea: ["otros"]
  },
  {
    nombre: "Plástico burbujas",
    descripcion: "Material de embalaje con burbujas de aire para protección.",
    imagen: "/images/products/plastico_burbujas.webp",
    slug: "plastico-burbujas",
    medidas: ["1.2 m x 10 m"],
    price: "$ 25.000",
    favorite: false,
    linea: ["plasticos"]
  },
  {
    nombre: "Plástico de colores",
    descripcion: "Láminas plásticas de colores para decoración y protección.",
    imagen: "/images/products/plastico_de_colores.webp",
    slug: "plastico-colores",
    medidas: ["1x5 m"],
    price: "$ 25.000",
    favorite: true,
    linea: ["plasticos"]
  },
  {
    nombre: "Plásticos para invernaderos",
    descripcion: "Plásticos especiales para cubrir y proteger cultivos en invernaderos.",
    imagen: "/images/products/plasticos-invernaderos.webp",
    slug: "plasticos-invernaderos",
    medidas: ["4x50 mts", "6x50 mts", "6x100 mts", "8x50 mts", "8x100 mts", "10x50 mts", "12x50 mts", "12x100 mts", "14x50 mts", "16x50 mts"],
    price: "$ 25.000",
    favorite: false,
    linea: ["agro"]
  },
  {
    nombre: "Polisombra",
    descripcion: "Malla sombra para protección contra el sol y viento.",
    imagen: "/images/products/polisombra.webp",
    slug: "polisombra",
    medidas: ["1x50 mts", "1x100 mts", "2x50 mts", "2x100 mts", "4x50 mts", "4x100 mts", "6x50 mts", "6x100 mts"],
    price: "$ 25.000",
    favorite: false,
    linea: ["agro"]
  },
  {
    nombre: "Stretch film",
    descripcion: "Película plástica para embalaje y protección de productos.",
    imagen: "/images/products/stretch_film.webp",
    slug: "stretch-film",
    medidas: ["5Pul x 350mts", "6Pul x 350mts", "12Pul x 350mts", "20Pul x 350mts"],
    price: "$ 25.000",
    favorite: true,
    linea: ["plasticos"]
  },
  {
    nombre: "Stretch film Negro",
    descripcion: "Película plástica para embalaje y protección de productos.",
    imagen: "/images/products/stretch_negro.webp",
    slug: "stretch-film-negro",
    medidas: ["5Pul x 350mts", "6Pul x 350mts", "12Pul x 350mts", "20Pul x 350mts"],
    price: "$ 25.000",
    favorite: true,
    linea: ["plasticos"]
  },
  {
    nombre: "Vinipel para Alimentos",
    descripcion: "Conserva la frescura y protege tus alimentos con nuestro vinipel resistente y de alta adherencia. Ideal para cocina y negocios, mantiene tus productos seguros y visibles.",
    imagen: "/images/products/vinipel.webp",
    slug: "vinipel-para-alimentos",
    medidas: ["50 mts", "100 mts", "200 mts", "300 mts", "500 mts"],
    price: "$ 25.000",
    favorite: true,
    linea: ["plasticos"]
  },
  {
    nombre: "Tapete alto tráfico",
    descripcion: "Tapete resistente para áreas de alto tránsito.",
    imagen: "/images/products/tapete_alto_trafico.webp",
    slug: "tapete-alto-trafico",
    medidas: ["1.2x1.8 m"],
    price: "$ 25.000",
    favorite: false,
    linea: ["otros"]
  },
];
